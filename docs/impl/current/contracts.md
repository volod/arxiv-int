...
- `arxiv-int db adopt` / `make db-adopt` live-adopts when that URL is set: relocates leftover
  ...
  catalogs, and stamps `0002` when the overlay includes ledger tables or `0001` for a complete
...
`0002` for the ctl run-ledger overlay documented in [Canonical store](canonical-store.md).
