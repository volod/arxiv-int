...
| --- | --- | --- | --- |
| `AUD-operator-entrypoints-1` | Observed / blocking before the short setup workflow can be advertised: README Quick Start requires eleven manual environment steps and no setup coordinator exists. `make bootstrap` syncs `.env`/`.venv` and audits readiness, but dependency sync, model and image acquisition, service start, schema preparation and readiness remain separate operator commands with no shared retry, no resumption of verified work and no per-phase status, so an operator editing `.env` between attempts cannot retry only what still fails. | [Retryable setup command](0022-runtime-implement-retryable-setup-command.md): prove absent `.env`/`.venv`, edited configuration, missing tools, failed sync/download/start, timeouts, cancellation and concurrent attempts, and that an unchanged retry reuses verified work while still probing readiness. | Resolved; [record 0022](0022-runtime-implement-retryable-setup-command.md) |
