...
| `AUD-codebase-10` | Observed baseline / blocking: required CI must pass before subsequent implementation acceptance | `runtime/__init__.py` import format/order and `tests/compose/test_profiles.py` complex combined topology assertion | Source/tests match HEAD. Repair formatting and split the test at invariant boundaries while retaining all assertions; then run the complete complexity gate and CI. | [Quality baseline repair](../plan.md#restore-quality-gate-baseline), routed |
