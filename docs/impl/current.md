...
| [Pipeline control](current/pipeline-control.md) | Typed stage/source/artifact interfaces, generic run ledger, fixture DAG CLI/Make, registered preflight worker, serialized progress logs, bounded resource telemetry, read-only pre-run forecast, profile-declared knowledge-base publication, run finalize, read-only stage artifact inspection, incremental source reconciliation, two-phase stale prune, read-only citation/source location lookup, provided-archive control proof, and the publication/reuse checkpoint; concrete corpus stages remain planned |
