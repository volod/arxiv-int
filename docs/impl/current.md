...
| [Evaluation](current/evaluation.md) | Operator-designated `ARCHIVE_DIR` slice; frozen fixture metrics, proof dispatcher, and the pipeline-control provided-archive proof exist; remaining provided-archive proofs and scale pilots stay planned |
...
| [Pipeline control](current/pipeline-control.md) | Typed stage/source/artifact interfaces, generic run ledger, fixture DAG CLI/Make, bounded content snapshots and metadata drift guards, registered preflight worker, serialized progress logs, bounded resource telemetry, read-only pre-run forecast, profile-declared knowledge-base publication, run finalize, read-only stage artifact inspection, incremental source reconciliation, two-phase stale prune, read-only citation/source location lookup, provided-archive control proof, and the publication/reuse checkpoint; inventory through chunk are registered and later investigation stages remain planned |
| [Corpus foundation](current/corpus-foundation.md) | Streaming inventory, tiered extraction, NFC/search views, reversible duplicate overlays, structure-aware chunks, content identities, archive policy, quarantine metadata, restart checkpoints, validated Parquet partitions and a current provided-archive integrity proof |
