...
| [Pipeline control](current/pipeline-control.md) | Typed stage/source/artifact interfaces, generic run ledger, fixture DAG CLI/Make, serialized progress logs, bounded resource telemetry, read-only pre-run forecast, profile-declared knowledge-base publication, run finalize, and the publication/reuse checkpoint; concrete corpus stages remain planned |
