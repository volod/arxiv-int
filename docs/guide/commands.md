...
| `arxiv-int search second-opinion --run-id RUN_ID --stage development\|preregister\|final [--json]` | available | Run one stage of the frozen Russian lexical second opinion: development (cost overrides allowed), preregistration of the final inputs, or the single final execution, each published below `$RUNS_DIR/<run-id>/evaluation/lexical-second-opinion/<stage>/`; `make lexical-second-opinion RUN_ID=... STAGE=...` is the wrapper. |
