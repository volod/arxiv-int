...
CONTRACT_FINGERPRINTS: dict[str, str] = {
    ...
    "file-classifications": "urn:arxiv-int:contract:file-classifications:1.0.0@1.0.0:edd4f1e0591ddce1a93fe4e069b7d5ade15b8d6fadcb0752919103a2bea07c0a",
...
def upgrade() -> None:
    ...
    op.create_table(
        "file_classification",
        sa.Column("classification_id", sa.Text(), nullable=False, comment=None),
        ...
        sa.Column("primary_class_id", sa.Text(), nullable=False, comment=None),
        sa.Column("alternate_class_ids_json", sa.Text(), nullable=False, comment=None),
        sa.Column("ancestor_path", sa.Text(), nullable=False, comment=None),
        sa.Column("confidence", postgresql.DOUBLE_PRECISION(), nullable=False, comment=None),
        sa.Column("calibration_profile", sa.Text(), nullable=False, comment=None),
        sa.Column("scores_json", sa.Text(), nullable=False, comment=None),
        sa.Column("evidence_json", sa.Text(), nullable=False, comment=None),
        sa.Column("failure_reason", sa.Text(), nullable=True, comment=None),
        sa.Column("extraction_fingerprint", sa.Text(), nullable=False, comment=None),
        sa.Column("normalizer_id", sa.Text(), nullable=False, comment=None),
        sa.Column("classifier_id", sa.Text(), nullable=False, comment=None),
        sa.Column("configuration_sha256", sa.Text(), nullable=False, comment=None),
        sa.Column("scheme_id", sa.Text(), nullable=False, comment=None),
        sa.Column("review_state", sa.Text(), nullable=False, comment=None),
        sa.Column("run_id", sa.Text(), nullable=False, comment=None),
        ...
        sa.PrimaryKeyConstraint("classification_id", name="pk_file_classification"),
        sa.ForeignKeyConstraint(["document_id"], ["corpus.documents.document_id"], name="fk_file_classification_document_id"),
        sa.ForeignKeyConstraint(["occurrence_id"], ["corpus.source_occurrences.occurrence_id"], name="fk_file_classification_occurrence_id"),
        schema="corpus",
        postgresql_partition_by="HASH (classification_id)",
        comment="Complete, evidence-backed subject classification of physical archive files, including explicit unclassified and unreadable outcomes and the fingerprints needed to reproduce each decision.",
    )
    for remainder in range(16):
        op.execute(
            f"CREATE TABLE corpus.file_classification_p{remainder:02d} "
            "PARTITION OF corpus.file_classification "
            f"FOR VALUES WITH (MODULUS 16, REMAINDER {remainder})"
        )
    op.create_index(
        "ix_file_classification_primary_class",
        "file_classification",
        ["primary_class_id"],
        schema="corpus",
    )
    op.create_index(
        "ix_file_classification_document_id",
        "file_classification",
        ["document_id"],
        schema="corpus",
    )
    op.create_index(
        "ix_file_classification_scheme_id",
        "file_classification",
        ["scheme_id"],
        schema="corpus",
    )
    op.execute(
        "CREATE UNLOGGED TABLE staging.classification_classes "
        "(LIKE corpus.classification_classes INCLUDING DEFAULTS INCLUDING COMMENTS)"
    )
    op.execute(
        "CREATE UNLOGGED TABLE staging.file_classification "
        "(LIKE corpus.file_classification INCLUDING DEFAULTS INCLUDING COMMENTS)"
    )
    op.execute(
        "GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON "
        "staging.classification_classes, staging.file_classification TO arxiv_int_pipeline"
    )
    op.execute(
        "GRANT SELECT ON staging.classification_classes, staging.file_classification "
        "TO arxiv_int_reader"
    )
...
def downgrade() -> None:
    ...
    op.drop_table("file_classification", schema="staging")
    op.drop_table("classification_classes", schema="staging")
    op.drop_table("file_classification", schema="corpus")
