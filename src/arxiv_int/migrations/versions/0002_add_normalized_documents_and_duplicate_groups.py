"""add normalized documents and duplicate groups
...
def upgrade() -> None:
    ...
    op.create_table(
        "duplicate_groups",
        sa.Column("duplicate_membership_id", sa.Text(), nullable=False, comment=None),
        sa.Column("group_id", sa.Text(), nullable=True, comment=None),
        sa.Column("document_id", sa.Text(), nullable=True, comment=None),
        sa.Column("method", sa.Text(), nullable=True, comment=None),
        sa.Column("role", sa.Text(), nullable=True, comment=None),
        sa.Column("score", postgresql.DOUBLE_PRECISION(), nullable=True, comment=None),
        sa.Column("suppressed", sa.Boolean(), nullable=True, comment=None),
        ...
        sa.PrimaryKeyConstraint("duplicate_membership_id", name="pk_duplicate_groups"),
        sa.ForeignKeyConstraint(["document_id"], ["corpus.documents.document_id"], name="fk_duplicate_groups_document_id"),
        schema="corpus",
        comment="Reversible duplicate and edition group memberships proposed over normalized documents.",
    )
    op.create_table(
        "normalized_documents",
        sa.Column("normalized_document_id", sa.Text(), nullable=False, comment=None),
        sa.Column("document_id", sa.Text(), nullable=True, comment=None),
        sa.Column("normalizer_id", sa.Text(), nullable=True, comment=None),
        sa.Column("language", sa.Text(), nullable=True, comment=None),
        sa.Column("language_confidence", postgresql.DOUBLE_PRECISION(), nullable=True, comment=None),
        sa.Column("normalized_sha256", sa.Text(), nullable=True, comment=None),
        sa.Column("text_chars", sa.BigInteger(), nullable=True, comment=None),
        sa.Column("generation_id", sa.Text(), nullable=False, comment=None),
        sa.Column("contract_version", sa.Text(), nullable=False, comment=None),
        sa.Column("bucket", sa.Text(), nullable=True, comment="Declared physical partition key from x-arxiv-int.partitionKey."),
        sa.PrimaryKeyConstraint("normalized_document_id", name="pk_normalized_documents"),
        sa.ForeignKeyConstraint(["document_id"], ["corpus.documents.document_id"], name="fk_normalized_documents_document_id"),
        schema="corpus",
        comment="Canonical normalized text views, detected language, and reversible offset maps.",
    )


def downgrade() -> None:
    """Reverse the frozen operations, or refuse when data cannot be restored."""
    op.drop_table("normalized_documents", schema="corpus")
    op.drop_table("duplicate_groups", schema="corpus")
